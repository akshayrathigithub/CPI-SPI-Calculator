...
// generate semester dynamically based on config defined in constant.js
function renderSemesterToDOM(semesterList, spiSemesterIndex = 0) {
  semesterList.forEach((sem, index) => {
    ...
    if (sem.includes("Summer")) {
      ...
      // grabbing course list of particular semester
    ...
    if (calculationContainer) {
      calculationContainer.before(divTag);
    }
  });
}
...
//methods to hide, show, disable and enable buttons, tags etc.
...
const fetchViewCounterInfo = async () => {
...
// recalculating grade, trigger page reload
